<?php

use Illuminate\Support\Facades\Route;

// --------------------------
// Custom Backpack Routes
// --------------------------
// This route file is loaded automatically by Backpack\Base.
// Routes you generate using Backpack\Generators will be placed here.

Route::group([
    'prefix'     => config('backpack.base.route_prefix', 'admin'),
    'middleware' => array_merge(
        (array) config('backpack.base.web_middleware', 'web'),
        (array) config('backpack.base.middleware_key', 'admin')
    ),
    'namespace'  => 'App\Http\Controllers\Admin',
], function () { // custom admin routes
    Route::crud('categories', 'CategoriesCrudController');
    Route::crud('site-settings', 'SiteSettingsCrudController');
    Route::crud('customers', 'CustomersCrudController');
    Route::crud('countries', 'CountriesCrudController');
    Route::crud('states', 'StatesCrudController');
    Route::crud('cities', 'CitiesCrudController');
    Route::crud('affiliates', 'AffiliatesCrudController');
    Route::crud('destinations', 'DestinationsCrudController');
    Route::crud('leads', 'LeadsCrudController');
    Route::crud('packages', 'PackagesCrudController');
    Route::crud('collections', 'CollectionsCrudController');
    Route::crud('collection-mappings', 'CollectionMappingsCrudController');
}); // this should be the absolute last line of this file